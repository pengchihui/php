...
<?php 
		...
		/*文本更新*/
//		function upda(){
//			$arry=json_decode(file_get_contents("music"),true);
//			
//		}
//		
...
<?php 
	    ...
	    if($picture['error']==4){
			 ...
			 /*文件删除*/
			unlink($result['picture']?$result['picture']:"");
	    ...
	    if($music['error']==4){
			 ...
			 /*文件删除*/
			 unlink($result['music']?$result['music']:"");
            @move_uploaded_file($music['tmp_name'],$muPath);
   	    }
	 		header("location: updatephp.php?id=".$id."&titile=".$titile."&singer=".$singer."&piPath=".$piPath."&muPath=".$muPath);
		    exit;
      } 
      /*关闭查询*/
